<x-layouts.app title="Edit Poli">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-xl font-bold text-slate-800">Edit Poli</h2>
    </div>
        ...
        <div class="p-7">
            ...
            <form action="{{ route('poli.update', $poli->id) }}" method="POST">
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="nama_poli" value="{{ old('nama_poli', $poli->nama_poli) }}" placeholder="Masukkan nama poli..." class="w-full border border-slate-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#2d4499] focus:border-transparent @error('nama_poli') border-red-500 @enderror" required>
                    @error('nama_poli')
                ...
                <div class="mb-6">
                    ...
                    <textarea name="keterangan" rows="4" placeholder="Masukkan keterangan poli..." class="w-full border border-slate-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#2d4499] focus:border-transparent resize-none @error('keterangan') border-red-500 @enderror" required>{{ old('keterangan', $poli->keterangan) }}</textarea>
                    @error('keterangan')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
